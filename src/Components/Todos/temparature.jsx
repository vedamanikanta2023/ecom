...
import dashBoardStyles from "../WeatherDashBoard/weatherDashboardStyles";
...
const upArrowIcon = <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-arrow-up-short" viewBox="0 0 16 16">
<path fill-rule="evenodd" d="M8 12a.5.5 0 0 0 .5-.5V5.707l2.146 2.147a.5.5 0 0 0 .708-.708l-3-3a.5.5 0 0 0-.708 0l-3 3a.5.5 0 1 0 .708.708L7.5 5.707V11.5a.5.5 0 0 0 .5.5"/>
</svg>;

const downArrowIcon = <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-arrow-down-short" viewBox="0 0 16 16">
<path fill-rule="evenodd" d="M8 4a.5.5 0 0 1 .5.5v5.793l2.146-2.147a.5.5 0 0 1 .708.708l-3 3a.5.5 0 0 1-.708 0l-3-3a.5.5 0 1 1 .708-.708L7.5 10.293V4.5A.5.5 0 0 1 8 4"/>
</svg>;

const locatoinIcon = <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="#ffffff" class="bi bi-geo" viewBox="0 0 16 16">
...
const Temp =(props)=>{
    ...
    return<Box display="flex" flexDirection={'column'} justifyContent="space-between" padding={5}>
        <div className={classes.tempContainer}>
            <Grid container className={classes.flexRow} style={{alignItems:"center",marginBottom:10}}>
                <Grid item xs={9} md={6}>
                    <Typography className={classes.mainTemp} textAlign={"start"}>
                        <span>{weatherIcon}</span>&nbsp;
                        {weather&&weather.temperature?`${Math.ceil(weather.temperature/10)}°C`:"NO Data"}
                    </Typography>
                </Grid>
                <Grid className={classes.tempContainer} item xs={3} md={3}>
                    <Box display="flex" justifyContent={"center"} alignItems={"flex-end"} flexDirection={'column'}>
                        <Typography className={classes.highLowTemps}>
                            <span>{upArrowIcon}</span>
                            {weather&&weather.temp_max?`${Math.ceil(weather.temp_max/10)}°C` :''}
                            
                        </Typography>
                        <Typography className={classes.highLowTemps} alignItems={"center"}>
        ...
        {props.user?<Typography className={classes.suggestion}>{`Dear ${(props.user).toUpperCase()}s, please plan you schedule according to the weather`}</Typography>:null}
